# State Manager 模块设计（v2 — 基于双 SOC 部署配置）

> **修订日期**：2026-05-21
> **修订原因**：基于实际部署配置 `sm.yaml`（双 SOC：Orin + x86）重新设计状态机，从"姿态导向"状态切换为"业务场景导向"状态
> **主要变更**：状态机从 17 状态精简为 16 状态 + 子状态机制；引入 Function Group State；增加双 SOC 部署架构描述；标准化动作语义（DIFF/activate/deactivate/close/fallback）
...
**定位**：SM 是端侧软件系统中的**全局状态权威**，位于中间件层。它维护机器人唯一的生命周期有限状态机（FSM），对外广播当前系统状态，对内校验所有状态转换请求的合法性。SM 只运行在 **Orin（Master SOC）** 上，通过 HTTP API 向 x86（Slave SOC）上的 EM 下发进程编排指令。
...
| 边界 | SM 负责 | 对方负责 |
|------|--------|---------|
| SM ↔ EM（Orin） | 状态转换决策，通过 HTTP API 下发进程编排指令 | Orin 上的进程生命周期管理 |
| SM ↔ EM（x86） | 通过 HTTP API 下发进程编排指令 | x86 上的进程生命周期管理 |
...
SM 只在 Orin（Master）上启动运行。x86（Slave）上的 EM 通过 HTTP API（`http://${AGIBOT_NEIGHBOR_HOST_IP}:50081`）接收来自 Orin SM 的进程编排指令。SM 的状态机是**全局唯一**的，两端的进程启停由同一个状态机决策。

| SOC | 角色 | SM 部署 | EM 部署 | 主要模块 |
|-----|------|---------|---------|---------|
| Orin（Master） | 主控 | ✅ SM | ✅ EM | Agent, Perception, SLAM, Gateway, Task Engine, MM, PnC |
| x86（Slave） | 运动控制 | ❌ | ✅ EM | MC, HAL_EtherCAT, Motion Player, Motion Streamer, TF, DR, RC, HDS |
...
> **EStop 的叠加语义**：`ESTOP` 不是一个独立的状态节点，而是可以叠加在任何状态之上的**安全标志**。当 EStop 触发时，当前业务状态保持不变（如仍标记为 MANUAL），但系统行为受 EStop 约束（禁止运动）。EStop 解除后，恢复到触发前的业务状态。
...
4. **跨 SOC 一致性**：状态转换时，SM 同时向 Orin 和 x86 的 EM 发送编排指令，两端任一失败则回滚
...
| **RTC** | rtc | 实时时钟 |
| **Poweroff** | poweroff0/1 | 关机处理（跨 SOC） |
| **Reboot** | reboot0/1 | 重启处理（跨 SOC） |
| **OTA** | fota_slave0/1 | 固件升级（跨 SOC） |
...
> **注意**：上表中的功能组映射来自实际部署配置 `sm.yaml`。各状态下的具体模块激活/停用通过 `action_list` 中的 DIFF 机制和显式 activate/deactivate 操作实现。
...
5. 通过 HTTP API 向 Orin 和 x86 的 EM 同时下发编排指令
6. 等待两端 EM 返回成功，状态转换完成
...
string active_modules_master     # Orin 上活跃模块列表（逗号分隔）
string active_modules_slave      # x86 上活跃模块列表（逗号分隔）
...
```mermaid
flowchart TB
    subgraph SMNode["state_manager_node（Orin）"]
...
- 加载 `sm.yaml` 中的 `function_groups` 和 `system_modes` 配置
- 将系统状态映射为模块列表（考虑双 SOC 分布）
...
- 考虑双 SOC 分布，将操作分别打包给 Orin EM 和 x86 EM

#### 7.2.4 HTTP Client（双 SOC 通信）

- Orin EM：`http://${AGIBOT_LOCAL_HOST_IP}:50081`
- x86 EM：`http://${AGIBOT_NEIGHBOR_HOST_IP}:50081`
- 状态转换时向两端同时发送编排请求
- **一致性策略**：两端都成功才算转换成功；任一端失败则回滚（向已成功的一端发送反向操作）

#### 7.2.5 E-Stop Handler

- 独立线程，与主 FSM 线程隔离
- 接收到 E-Stop 请求后立即设置 `estop_active=true`
- 通过 HTTP 向两端 EM 发送 MC 停止指令（优先级最高）
...
RequestTransition(target_state=MANUAL)
  ...
  → HTTP Client：
      同时向 Orin EM 和 x86 EM 发送 ApplyProcessSet 请求
      等待两端返回（超时 30s）
  → 两端都成功：
...
触发源 → SM E-Stop Handler
  ...
  → 通过 HTTP 向 Orin EM 发送：停止 MC 进程
  → 通过 HTTP 向 x86 EM 发送：停止 MC 进程
...
5. **跨 SOC 一致性**：状态转换涉及双端 EM，任一端失败必须回滚，禁止"半转换"状态
...
| 1005 | ERR_EM_ORCHESTRATION_FAILED | Orin EM 编排失败 | CRITICAL |
| 1006 | ERR_EM_SLAVE_ORCHESTRATION_FAILED | x86 EM 编排失败 | CRITICAL |
...
| 1013 | ERR_HTTP_TIMEOUT | 向 x86 EM 发送请求超时 | HIGH |
...
sm/                           # 节点实现包
    include/sm/
        ...
        http_client.hpp             # 双 SOC HTTP 客户端
    src/
...
| 普通状态转换处理延迟 | < 500ms | 含双端 EM 编排确认 |
| 单端 EM 编排延迟 | < 200ms | Orin 或 x86 单独编排 |
| `/sm/is_motion_allowed` 响应延迟 | < 1ms | 本地原子变量读取 |
| `/sm/robot_state` 发布延迟 | < 1ms | 转换完成后广播 |
| x86 EM HTTP 请求往返延迟 | < 50ms | 局域网内跨 SOC 通信 |
| HTTP 超时 | 30s | 向 x86 EM 发送编排请求的超时时间 |
| SM 心跳抖动 | < 50ms | — |
| SM 自身 CPU 占用 | < 0.5% | — |
| SM 自身内存占用 | < 50MB | 含配置缓存和模块状态 |
| 系统启动到 READY 状态 | < 60s | 含双端模块编排 |
