...
```mermaid
stateDiagram-v2
    [*] --> CONNECTED : SLAVE_REGISTER 成功
    CONNECTED --> HEARTBEAT_MISS : 心跳丢失 1 次
    HEARTBEAT_MISS --> CONNECTED : 心跳恢复
    HEARTBEAT_MISS --> DISCONNECTED : 连续 ≥3 次丢失
    CONNECTED --> AUTONOMOUS : Slave 通告自治
    CONNECTED --> DISCONNECTED : MQTT Will 触发（Broker 通告断联）
    DISCONNECTED --> RECONNECTING : Slave 重新连接
    AUTONOMOUS --> RECONNECTING : Slave 重新连接
    RECONNECTING --> CONNECTED : 缓冲数据回放完成
```
...
sequenceDiagram
    ...
    Note over Slave: MQTT CONNECT (CleanSession=false)
    Slave->>Broker: CONNECT (Will: status="offline", Retain=true)
    Slave->>Broker: PUBLISH hds/slave/{id}/status (status="online", Retain=true, QoS 1)
    Broker->>Master: hds/slave/{id}/status (Retained)
    Master->>Master: SlaveRegistry 注册新 Slave
    Master->>Broker: PUBLISH hds/slave/{id}/command (配置下发, QoS 1)
    Broker->>Slave: hds/slave/{id}/command

    loop 正常运行
        Slave->>Broker: hds/slave/{id}/heartbeat (QoS 0, 1Hz)
        Broker->>Master: hds/slave/{id}/heartbeat
        Master->>Broker: hds/master/heartbeat (QoS 0, 1Hz)
        Broker->>Slave: hds/master/heartbeat
    end

    Note over Slave: 网络中断
    Broker->>Broker: Will 超时（keepalive × 1.5）
    Broker->>Master: hds/slave/{id}/status (status="offline", Retained)
    Master->>Master: Slave → DISCONNECTED（即时感知）
```
...
```mermaid
flowchart TB
    subgraph MasterNode["HDS Master Node"]
        subgraph DataLayer["数据层"]
            LocalCollector["LocalCollector
            SOC-0 本地数据采集
            · /hds/health_report 订阅
            · /hds/register_entity 服务"]

            MqttBridge["MqttBridge
            MQTT 通信层
            · 订阅 hds/slave/+/#
            · 发布 hds/master/*, hds/slave/{id}/command
            · Will Message 断联检测"]

            SlaveAggregator["SlaveDataAggregator
            · 解析 health_batch / critical JSON
            · entity_id 加 soc 前缀
            · 标准化为内部 HealthReport"]

            MqttSubscriber["MqttSubscriber
            · em/event/l3_suggestion
            · em/event/l4_suggestion"]
        end
        ...
        subgraph DiagLayer["诊断层"]
            DiagEngine["GlobalDiagEngine
            · RuleEvaluator（本地 + 跨 SOC 规则）
            · TrendAnalyzer（滑动窗口趋势）
            · SeverityMapper（定级映射）"]
        end

        subgraph DecisionLayer["决策层"]
            Arbiter["StateTransitionArbiter
            · 去抖防抖
            · SM 请求决策
            · E-Stop 快速路径"]
            ...
            RecoveryHandler["RecoveryCheckHandler
            · 检查本地诊断实体
            · 检查所有 Slave 连接状态
            · 2s 超时硬限制"]
        end
        ...
        subgraph OutputLayer["输出层"]
            StatePub["DiagnosisState Publisher"]
            AlarmPub["AlarmEvent Publisher"]
            ChainPub["DiagnosisChain Publisher"]
            SummaryPub["GlobalDiagnosisSummary Publisher (1Hz)"]
            SlavePub["SlaveStatus Publisher"]
            HeartbeatTimer["Heartbeat Timer (1Hz)"]
        end

        LocalCollector --> EntityRegistry
        LocalCollector --> DiagEngine
        SlaveAggregator --> EntityRegistry
        SlaveAggregator --> DiagEngine
        MqttSubscriber --> DiagEngine
        MqttBridge --> SlaveRegistry
        MqttBridge --> SlaveAggregator
        SlaveRegistry --> DiagEngine
        DiagEngine --> Arbiter
        DiagEngine --> AlarmMgr
        Arbiter --> StatePub
        AlarmMgr --> AlarmPub
...
```mermaid
sequenceDiagram
    participant MC as MC (SOC-1)
    participant Slave as HDS Slave-1
    participant Master as HDS Master
    participant SM
    participant GW as Gateway

    MC->>Slave: /hds/health_report (cpu_percent=95%)
    Slave->>Slave: LocalDiagEngine 预过滤
    Slave->>Master: MQTT hds/slave/{id}/health_batch (QoS 0)
    Master->>Master: GlobalDiagEngine 评估
    Master->>Master: 定级 WARNING
    Master->>Master: 发布 /hds/diagnosis_state

    MC->>Slave: /hds/health_report (bus_error_count=5)
    Slave->>Slave: LocalDiagEngine → severity=FAULT
    Slave->>Master: MQTT hds/slave/{id}/critical (QoS 1, 立即)
    Master->>Master: GlobalDiagEngine 评估 → FAULT
    Master->>Master: 跨 SOC 规则: ethercat_fault + motion_active → E-Stop
    Master->>SM: /sm/trigger_estop (priority=100)
    SM-->>Master: accepted
    Master->>GW: /hds/alarm_event (critical)
```
