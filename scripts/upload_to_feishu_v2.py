#!/usr/bin/env python3
"""Upload markdown to Feishu docx using only text and heading blocks."""
...
def parse_markdown(content):
    ...
    while i < len(lines):
        ...
        if stripped.startswith('```'):
            ...
            i += 1
            prefix = f"[{lang}]\n" if lang else ""
            blocks.append({"type": "code", "content": prefix + '\n'.join(code_lines)})
            continue
        ...
        # Divider -> skip (we'll use visual separation via headings)
        ...
        if bullet_match:
            ...
            while i < len(lines):
                ...
                if sl.startswith('* ') or sl.startswith('- '):
                    items.append('• ' + sl[2:])
                    i += 1
                elif sl and not sl.startswith('#') and not sl.startswith('|') and not sl.startswith('```') and not sl.startswith('---'):
                    if items:
                        items[-1] += '\n' + sl
                    i += 1
                else:
